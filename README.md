...
|---|---|
| `start_here` | Coming online: getting the world model, the skills that exist by name and when-to-use, a live snapshot of what exists, and — when naming an identity — that identity's rules, mailbox counts, a session handle, and its charter on a first boot but not on a resume. Naming a skill returns that one procedure. **A run may name its TIMEZONE and the DAY it is working in here, and everything day-grained resolves against both** — every write, every day-grained read, the staleness sweep and the offer window. A call naming its own date still wins; a run that names no zone is answered in UTC and one that names no day is answered on the clock, each said rather than applied silently. **jojobot never derives the day and never advances it.** ⭐ **The server itself may be acting out a day** — then that is the clock a run without one is answered on, **and this answer says so and tells the caller to name a day only if theirs differs.** The one door; there is deliberately no second. |
...
| `update_entity` | Maintaining what a thing is called, and its other metadata. It does not move the handle; `rename_entity` does, and the refusal says so. |
| `rename_entity` | Calling a thing something else — a slug, a kind or a parent — **without breaking a single link to it.** Everything that points at a thing holds its permanent id rather than its name, so nothing is rewritten and every old reference still resolves. A handle the thing used to wear keeps working. |
| `merge_entities` | Folding two records of the same thing into one, when a thing was written down twice. Pointers drawn at the folded side are repointed at the survivor, including one written against a handle that side used to wear. **A handle somebody wrote down goes on resolving; the address of a record that moved says where it went rather than answering as though nothing happened.** |
| `update_fact` | Correcting something recorded wrong — rewritten in place, never as an addendum. Also how a claim gets confirmed, or a refutation recorded as standing truth. **The wording it replaced is KEPT** and readable through `recall` with a record's address, so a claim you disagree with is safe to correct: you are not deciding whether the old wording survives, only what the claim says now. **When a rewrite turns a claim about an event into its negation, the receipt names the fork** — retract if the record was never true, this rewrite if it was true and has changed — and never guesses which. |
...
| **Compare two claims that disagree** | A claim's own writes are kept and readable, so what a record used to say before somebody corrected it is one read. What is not served is asking jojobot which of two accounts is right: it stores and retrieves, and the agent does the judging. |
...
| **Notice something between a caller's questions** | Every read answers a question somebody asked. Nothing watches the store on its own and nothing reaches out, so a thing that quietly stops being true waits for a caller to go looking. |
