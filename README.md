...
| `declare_type` | Naming the set of keys a thing carries, so a key means the same thing everywhere and the graph can be filtered on it. **A THING answers a type**, over every write on it folded together — so things written before the type was declared answer it too. **The software ships its own vocabulary, `rhythm` and `trip`, declared at startup**; a shipped name is closed to a caller's redeclaration, and every type says where it came from. |
