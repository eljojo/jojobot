...
**The words live in `crates/jojobot-exercise/src/handover_room.rs`, once.**

## The locks

1. **The brief left the box.**
2. **A colleague exists, and its box came with it.** A box is not made — it
   opens with the bot that owns it, in the one act — and the occupant is never
   told that. The positive it rests on: the shipped identity is on the board,
   so an unread answer cannot read as a missing colleague. Two colleagues where
   the brief asked for one is a fail: the room asked for a second assistant,
   not for a habit of making them.
3. **The pile is in the colleague's box, one thing at a time.** The brief asks
   for them separately so they can be worked separately. A run that handed the
   whole pile over as one message has left a colleague one thing to finish
   rather than three. Paired with the positive that they came from the occupant
   rather than from nowhere.
4. **How much of the pile is still waiting is on the colleague.** The terminal
   lock. Both halves: the count is written where the operator asked for it, and
   the pile really is untouched — so a run that wrote a number without looking
   is not credited with an answer that happens to be right.

**The count is accepted as a figure or as the word.** Which one a session
writes is not what this room measures.
