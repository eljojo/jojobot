...
--
-- **`home` and `id` are the address a caller edits through**, so they are the
-- key. `home` is where the claim lives and `subject` is what it is about; they
-- are separate columns because they can legitimately differ — a claim about one
-- entity may be filed on another's page — and `recall` answers for both.
--
...
--
-- `derived_from_home` and `derived_from_id` are the address of the claim this
-- one came from, when it came from a claim rather than from an entity. Two
-- columns rather than one string, so the pair is addressable the way every
-- other address here is.
CREATE TABLE fact (
    ...
    derived_from_home VARCHAR(191) NULL,
