...
impl Jojobot {
    ...
    /// **The gate key for the operator's Journal page — not any session's.**
    ///
    /// Every other key names one identity. This one names the single document
    /// every wrap publishes into, because the callers who must exclude each
    /// other there are DIFFERENT BOTS — which is exactly the pair an identity
    /// key files in separate queues.
    ///
    /// It cannot collide with a real key: a bot's key always carries a `:`, and
    /// a handle is exactly [`jojobot_domain::session::SID_LEN`] characters with
    /// no `-` in the alphabet at all.
    const JOURNAL_GATE: &'static str = "the-journal-page";
    ...
    async fn wrap_session(
        ...
        // **Publishing is serialized on the PAGE, and taken second.**
        //
        // The Journal is one document shared by every session there has ever
        // been, and appending to it is a read-modify-write over the whole text:
        // read it, add an entry at the end, write it all back, read it back.
        // Two wraps overlapping in that span lose an entry — and the one whose
        // read-back comes up wrong rolls the page to the snapshot it read,
        // erasing the entry the other had already committed and verified. Both
        // sessions then close `wrapped` believing they published, and `wrapped`
        // is the last word, so neither story can ever be told again.
        //
        // The gate above cannot cover it: that one keys on the identity, and
        // the two callers here are two identities. What they share is the page.
        //
        // **Taken second, and this is the only place this key is taken** — so
        // the lock order is uniform by construction and there is nothing to
        // deadlock against. Held through the close, because a story reaching
        // the page and a session saying it told one have to become true
        // together.
        let publishing = self.registry.gate(Self::JOURNAL_GATE);
        let _published_alone = publishing.lock().await;
...
mod tests {
    ...
    /// A Memory whose Journal is the ONE page a real store serves: read the
    /// whole document, add an entry at its end, write the whole document back,
    /// then read it back — and roll the page to what THIS caller read if what
    /// comes back is not what it wrote. That is `outline.rs`'s `append_journal`,
    /// modelled rather than described.
    ///
    /// **`InMemoryMemory` cannot lose a Journal entry**: its append is a
    /// `Vec::push` under a mutex, atomic against anything that could interleave
    /// with it. Two bots wrapping at once both land against that fake, so a test
    /// asserting both stories survive passes on broken code and proves nothing.
    ///
    /// The suspension points are the two a round trip has — after the read and
    /// after the write — because both are observable on the server before the
    /// caller resumes. Neither is optional: without the second, a clobbered
    /// read-back is unreachable and the rollback never fires.
    struct PagedJournal {
        inner: Arc<InMemoryMemory>,
        page: std::sync::Mutex<String>,
    }

    impl PagedJournal {
        fn new(inner: Arc<InMemoryMemory>) -> Self {
            Self {
                inner,
                page: std::sync::Mutex::new(String::new()),
            }
        }
        ...
        fn read(&self) -> String {
            self.page
                .lock()
                .expect("the page mutex is poisoned")
                .clone()
        }

        fn write(&self, text: &str) {
            *self.page.lock().expect("the page mutex is poisoned") = text.to_string();
        }
    }

    #[async_trait]
    impl Memory for PagedJournal {
        async fn append_journal(
            &self,
            on: jiff::civil::Date,
            entry: &str,
            ...
            tokio::task::yield_now().await;
            let appended = match snapshot.trim().is_empty() {
                true => stored.clone(),
                false => format!("{}\n\n{stored}", snapshot.trim_end()),
            };
            self.write(&appended);
            tokio::task::yield_now().await;
            let seen = self.read();
            if !seen.contains(&stored) || !seen.contains(snapshot.trim()) {
                // The adapter's own move: put back what this caller read, which
                // is what makes a lost update into two lost updates.
                self.write(&snapshot);
                return Err(MemoryError::Store("the Journal read back changed".into()));
            }
            Ok(stored)
        }

        /// The Journal scans as a document, exactly as it does in the store —
        /// `journal_holds` reads it through here and nowhere else. The inner
        /// fake never sees an append, so it contributes no Journal doc of its
        /// own to collide with this one.
        async fn scan(&self) -> Result<Vec<jojobot_domain::memory::search::DocScan>, MemoryError> {
            let mut docs = self.inner.scan().await?;
            let page = self.read();
            if !page.trim().is_empty() {
                docs.push(jojobot_domain::memory::search::DocScan {
    ...
    /// **Two bots wrapping at once must not destroy each other's story.**
    ///
    /// Wrapping publishes: the story goes onto the operator's Journal, one page
    /// shared by every session there has ever been, and appending to it is a
    /// read-modify-write over the whole document with no lock and no CAS. Two
    /// wraps overlapping there lose an entry outright — and worse, the caller
    /// whose read-back comes up wrong rolls the page to the snapshot it read,
    /// erasing the entry the other one had already committed and verified. Both
    /// sessions then close `wrapped`, believing they published, and `wrapped` is
    /// the last word: neither story can ever be told again.
    ///
    /// **The bot gate cannot cover this**, and that is the whole point — it
    /// keys on the identity, and these are two identities. What they share is
    /// the page, so the page is what the second gate keys on.
    ///
    /// The operator runs two bots at once by design, so this is reachable
    /// rather than theoretical.
    #[tokio::test]
    async fn two_bots_wrapping_at_once_both_reach_the_journal() {
        for gamma_first in [true, false] {
            let inner = Arc::new(InMemoryMemory::new());
            let paged = Arc::new(PagedJournal::new(inner));
            let jojobot = Jojobot::new(
                paged.clone(),
                Arc::new(SpySearch::default()),
                Arc::new(InMemoryMailboxes::new()),
            ...
            make_bot(&jojobot, "gamma", None).await;
            make_bot(&jojobot, "delta", None).await;
            // **The page starts with an entry on it**, which is what makes the
            // rollback reachable: an empty snapshot is a substring of every
            // page, so the read-back guard cannot tell it was overwritten.
            paged.write("## 2026-07-26\n\nwhat some earlier run did");

            let mine = booted(&jojobot, "gamma").await;
            let theirs = booted(&jojobot, "delta").await;
            let wrapping = |sid: String, story: &'static str| {
                jojobot.wrap_session(Parameters(WrapSessionArgs {
                    story: story.into(),
                    sid,
                }))
            };
            let g = wrapping(mine, "gamma finished the slice it was given");
            let d = wrapping(theirs, "delta answered the mail and stopped");
            let (first, second) = match gamma_first {
                true => tokio::join!(g, d),
                false => {
                    let (d, g) = tokio::join!(d, g);
                    (g, d)
                }
            };
            let page = paged.read();
            assert!(
                ...
                "gamma_first={gamma_first}: a wrap failed outright, leaving the page {page:?}"
            );

            for published in [
                "what some earlier run did",
                "gamma finished the slice it was given",
                "delta answered the mail and stopped",
            ] {
                assert!(
                    page.contains(published),
                    "gamma_first={gamma_first}: the Journal lost {published:?}, which a wrapped \
                     session can never tell again: {page:?}"
                );
            }
        }
    }
