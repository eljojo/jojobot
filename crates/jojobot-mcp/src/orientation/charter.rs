...
/// here so a future writer and the view that reads it cannot drift apart on
/// the spelling.
///
/// **No default ships under it yet.** `Provision::record` — the mechanism
/// `views.rs` uses to ship a whole record — supplies a record the store holds
/// NOTHING of; `bot:assistant` is a real stored row, so also supplying one
/// here made `ensure_default_identity`'s existence check see the identity as
/// already provisioned and skip creating its real row and mailbox, which
/// broke two user stories on the real store. Shipping a default one-liner for
/// an identity that already has a stored row needs its own mechanism — narrow
/// enough not to trip that check — and is not this batch's to build.
