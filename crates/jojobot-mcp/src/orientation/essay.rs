...
**MEMORY** is a typed graph of the operator's life. An **entity** is a noun — person · project · place · event · work · thing · org · topic · bot · pet · rhythm — with a handle like `person:milhouse`, the name it is addressed by and the only name a caller ever sends. A **rhythm** is a recurring loop and it is the one kind that REQUIRES a `parent`, which says whose job the loop is: a maintenance loop under the thing maintained, a review loop under the bot that carries it. Two loops on one object are two rhythms, which is why they are entities rather than a label. A **fact** is one dated claim about an entity, addressed `person:milhouse#3`, carrying a **provenance**: `testimony` (the operator said or confirmed it), `observation` (an AI read it out of a system of record and says which one) or `inference` (an AI derived it). Inference is the default and reads back as a hypothesis, never as truth; only the operator's explicit confirmation promotes a claim. **An `observation` reads back as a finding rather than a hypothesis, and must name where it was read** — the field `read_from`, plus `read_ref` for what was read there if you have it — because who confirmed something and on what is one question, and a claim answering half of it is refused. A fact may draw one typed **edge** at another entity — `location` · `membership` · `attendance` · `about` · `connection` — and edges are what make cross-entity questions answerable. `connection` is the one to reach for when a link is there and how it relates was not recorded: filing that as `about` would state something nobody said. **`search` is the front door** to all of it — and to the messages in mailboxes too, when you ask for them with `include_mail: true`: one ranked list, one call.

**A record carries FIELDS**, always: a flat bag of key/value pairs beside the claim, which jojobot stores and never interprets. Nothing has to be declared before you write one, and a key you invent is kept exactly as you wrote it. **A thing's fields are every write on it, folded, the newest write of each key winning** — what jojobot knows about `person:milhouse` is the claims plus the keys written onto it a piece at a time, and a write that takes a key off takes it off the thing. **Carrying keys is what makes a thing ANSWER a type**, and answering one is not being held to one. Declare a type to say which keys it names; a thing holding all of them FITS it. Declaring admits nothing and refuses nothing — a thing is found by the keys it carries whether or not anybody declared the type, and no write is ever blocked by a type. **What a write may take off a thing is its KIND's question**: a kind may name keys that the things of that kind have to keep, and a write that would drop one of those is blocked, naming the key it would lose. No shipped kind names any key, so nothing is held to anything yet. What declaring a type buys is write-time help, the vocabulary search asks with, plus ordering and traversal on the keys it names. Two questions, and the difference is the point: `answers_type` selects things carrying SOME of a type's keys and says which each one lacks, for finding what is worth looking at; `fits_type` keeps only the things with no gaps. Ask `answers_type` for *which of these are described like a pet, and what is missing*, and `fits_type` for *which of these ARE pets*.
