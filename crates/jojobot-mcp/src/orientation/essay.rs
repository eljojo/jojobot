...
**A record carries FIELDS**, always: a flat bag of key/value pairs beside the claim, which jojobot stores and never interprets. Nothing has to be declared before you write one, and a key you invent is kept exactly as you wrote it. **A thing's fields are every write on it, folded, the newest write of each key winning** — what jojobot knows about `person:milhouse` is the claims plus the keys written onto it a piece at a time, and a write that takes a key off takes it off the thing. **Carrying keys is what makes a thing a type.** Declare a type to say which keys it names; a thing holding all of them FITS it. Declaring admits nothing — a thing is found by the keys it carries whether or not anybody declared the type — and refuses one move only: a write that would drop a thing below a type it already fits is blocked, naming the key it would lose. What declaring buys is write-time help, plus ordering and traversal on the keys it names. Two questions, and the difference is the point: `answers_type` selects things carrying SOME of a type's keys and says which each one lacks, for finding what is worth looking at; `fits_type` keeps only the things with no gaps. Ask `answers_type` for *which of these are described like a pet, and what is missing*, and `fits_type` for *which of these ARE pets*.
