...
- *"Remember the diner does a Thursday special — I read it on their posted menu"* → the operator gave you a claim AND where it came from. Record both. `add_entity` the source first, because it is a thing in its own right: `thing:leftorium-menu`. Then `capture` subject `place:leftorium`, content the claim, provenance `testimony`, with edge `{shape: about, object: thing:leftorium-menu}`. **A source the operator names is not decoration.** Record it in the same write, or it is gone and the claim becomes one nobody can check.
...
- *Leave word for another session* → the snapshot this door hands you names every box on the board, so you already know what exists; `post_message` into an agreed one, with a body written for a reader with none of your context. jojobot records who sent it from the `sid` you pass, so there is nothing to declare and nothing to get wrong.
...
A **blocked** result is a SUCCESS whose body says `status: "blocked"`, `wrote: false`: nothing was written, and `how_to_proceed` says what to do next. Never retry one unchanged. Five gates produce it, with different ways out: **resemblance** (creating or renaming something that looks like what exists — pick the candidate you meant, or `create_new: true` only when you can say how the two differ; an exact handle or box name is never overridable), **absence** (you named something that is not there — the subject of a capture, an edge's object, the box of a post, a handle to read, an address to edit, a message id to retire; empty `candidates` means nothing even resembles it, not that your call was malformed; for an entity, creating it and retrying is usually right — for a mailbox it usually is not), **ownership** (a mailbox has exactly one owner, and a second claim on one is refused naming the holder; `create_new` does not clear this — it answers a question about names), **unreadable** (`mark_processed` reached an item jojobot cannot read — no retry helps, a person must repair it; treat what it carried as unhandled and say so), and **shape** (a field is validated and what you sent is not what it can carry — a message's `subject` is one plain line of unformatted text; the answer says what the rule is, and the way out is to send the same call with that field fixed or left out).
...
This door's snapshot names every box on the server: that is a fact about the board and **not an invitation**. Only your own comes back with counts — somebody else's queue is not yours to weigh — and even the bare name of a box is not an address you are meant to work. If you need something from another box, ask its owner or leave a message in it — `post_message` writes without reading, which is exactly the shape of a request.
"#;
