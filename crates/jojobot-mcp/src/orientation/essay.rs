...
/// that a question can be asked structurally, the verbs a cold session
/// writes with, its own zone and day, and the call that reaches the rest.
/// Engine prose: the method, in role language only — no operator specifics,
/// fictional example identities.
///
/// **The boundary is drawn by what a cold session cannot act without, not
/// by what teaches the model best.** A first pass drew it at the essay's own
/// paragraph seam — kinds and fields, nothing past it — and three
/// pre-existing user stories (`unprompted`, `backdated`, `timezones`, in
/// `crates/jojobot/tests/user_stories/`) caught what that missed: a named
/// boot that never learns `add_entity`/`capture`/`recall` exist cannot write
/// anything, and one that never learns `timezone`/`today` cannot date a
/// claim correctly. Those stories are the acceptance criteria for this
/// boundary — they are pre-existing, realistic, already-approved assertions
/// about first contact, not this file's own taste.
...
/// sessions. **A named boot never gets this at all** — it is not a ranking
/// candidate for one, only a rule. An anonymous boot gets it joined with the
/// core, as the one candidate ranked against the boot's declared ceiling —
/// see `essay_for_boot` in `jojobot-mcp`'s `orient` module.
...
- *"These three claims are really saying one thing now — mark the newest as standing for the others"* → `update_fact` the newest, with `stands_for` naming the older claims' addresses. This is a synthesis, not a citation: the named claims stay exactly as they are, active and readable. `recall`'s facts listing leaves them out once the mark lands; ask again with `stood_for: true` for the sources. `clear_stands_for` takes the mark off. A mark naming an address that does not exist, the record's own address, one already a shape, or an empty set is refused.
...
- *"I mistyped that just now"* → `recall` the subject, then `update_fact` rewrites the claim in place — the record becomes what it should have said, and the wrong wording stays readable through history. *"That changed"* or *"that was never true"* is a different move: the old claim was so once, or never was — `update_fact` the old claim with `status: archived` and `details` saying why ("confirmed by the operator: not allergic after all"), then `capture` the new one, naming the archived claim as its `derived_from` when there is a direct replacement. Archive it, never negate it: rewriting a claim into its own denial ("the club does NOT meet on Tuesdays" replacing "the club meets on Tuesdays") leaves a sentence about what is not so where a claim about what is true belongs. A negative is still an ordinary fact when it is not correcting anything — "he did not attend" stands on its own.
