...
- *"That never happened"* → **an event is not fixed, it is retracted.** A fact is current truth and gets rewritten in place; an event is chronology, it stays put, and `retract` takes it back: the record keeps its address and its words and is marked, and a dated account of why lands beside it, so the two read as one story. It is ONE WAY — nothing un-retracts, not a second call and not an edit — so if you are unsure, capture what is so now instead. The dividing line is the same one that decides what an event is in the first place: if you find yourself wanting to edit a record, it was never an event.
